...
//prototyping
void solve(sudoku_one::board &lev,int end = 0, int count = 1);
...
//SOLVE function
void solve(sudoku_one::board &lev, int end, int count) {
    // if valid answer then put it in and move to the next if not valid
    ...
    if (*p == 99) {
        ...
        if (count == 0) {
            ...
            if(count == 0){
                count = finder(i,j,lev);
            }
            std::cout << i << j << "    count: "<< count <<std::endl;
...
int finder(int i, int j, sudoku_one::board &lev, int count) {
    ...
    if (count > 9) {
        std::cout <<i << j << "oh boy" << count<< std::endl;
        return 0;
    } else {
        return count;
    }
